...
for i in range(len(TEST)):
        ...
        a=Dictionnaire["Date de naissance"].split("/")
        if valide_num(Dictionnaire["Numero"]) and Nom(Dictionnaire["Nom"]) and Prenom(Dictionnaire["Prenom"]) and Valide_Classe(Dictionnaire["Classe"]) and Valide_date(int(a[0]),int(a[1]),int(a[2])):
	        LISTE_Valide.append(Dictionnaire)
        if not (valide_num(Dictionnaire["Numero"]) and Nom(Dictionnaire["Nom"]) and Prenom(Dictionnaire["Prenom"]) and Valide_Classe(Dictionnaire["Classe"]) and Valide_date(int(a[0]),int(a[1]),int(a[2]))) :
            LISTE_Invalide.append(Dictionnaire)     
            inv = Element('LISTE_INVALIDE')
            for i in LISTE_Invalide:
                inv_element = SubElement(inv,'i')
                for key, value in i.items():
                    sous_ele_inv = SubElement(inv_element, key)
                    sous_ele_inv.text = value
with open("LISTE_VALIDE_projet.json","w") as fichier_json:
	        	json.dump(LISTE_Valide, fichier_json)
with open('LISTE_Invalide.xml', 'wb') as fichier_xml:
                try:
                    # Convertir le nombre float en une chaîne de caractères
                    inv.attrib['Prix'] = str(10.89)
                    # Écrire l'élément XML dans le fichier
                    fichier_xml.write(tostring(inv))
...
while True:
    ...
    if choix == "1":
        ...
        print(tabulate(LISTE_Valide, headers="keys", tablefmt="psql"))
    elif choix == "5":
        break
    else:
